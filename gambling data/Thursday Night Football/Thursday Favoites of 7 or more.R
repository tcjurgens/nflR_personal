...
# filter for home favorites of 7 or more points
thursday_favorites <- thursday_games %>%
  filter(spread_line <= -7) 

covers <- sum(thursday_favorites$ATS_win, na.rm =TRUE)
# in the 58 instances that there has been a thursday night favoite of 7 or more points, 37 times have they covered the number

outright_losers <- thursday_favorites %>%
  filter(result > 0)
